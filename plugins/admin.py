...
async def admin_panel(client, message):
    ...
    # 🌟 Verification Time text format (Hours me convert karke dikhane ke liye)
...
async def admin_callback(client, query):
    ...
    # 🌟 NEW: Token Expiry Validity Listener 
    elif action == "set_token_time":
        await query.message.delete()
        time_msg = await client.ask(query.message.chat.id, "🔑 **Token Validity ka time Hours (Ghante) me bhejein:**\n*(Example: Agar 24 ghante validity chahiye to sirf 24 likhein. Agar 1 ghanta chahiye to 1 likhein)*")
        try:
            hours = int(time_msg.text)
            seconds = hours * 3600
            await db.update_setting("verify_expire_time", seconds)
            await client.send_message(query.message.chat.id, f"✅ Token validity badal kar **{hours} Hours** kar di gayi hai! Dubara /admin type karein.")
        except ValueError:
            await client.send_message(query.message.chat.id, "❌ Invalid format! Sirf number bhejna tha.")
        return
